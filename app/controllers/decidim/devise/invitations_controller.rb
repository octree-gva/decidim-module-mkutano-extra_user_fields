...
module Decidim
  module Devise
    ...
    class InvitationsController < ::Devise::InvitationsController
      ...
      def accept_resource
        @form = Decidim::RegistrationForm.from_params(params[:user]).with_context(
          current_organization: current_organization, 
          current_user: resource  
        )
        extended_data = @form.attributes
        resource = resource_class.accept_invitation!(update_resource_params)
        unless resource.valid?
          resource.organization = current_organization
          resource.extended_data = extended_data
          resource.name = "#{extended_data[:first_name]} #{extended_data[:last_name]}"
          resource.nickname = params.require(:user).require(:nickname)
          resource.password = params.require(:user).require(:password)
          resource.password_confirmation = params.require(:user).require(:password_confirmation)
          resource.save
          resource = resource_class.accept_invitation!(update_resource_params)
        end
      ...
      def configure_permitted_parameters
        devise_parameter_sanitizer.permit(:accept_invitation, keys: [:nickname, :tos_agreement, :newsletter_notifications, :email])
      end
